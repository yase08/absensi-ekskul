...
const Login = () => {
  ...
  return (
    <div className='w-full h-[100vh] bg-primary flex relative transition-all '>
      ...
      >
        <div className={` w-full h-full px-5 ${changeMethod ? 'opacity-0 hidden' : 'opacity-100'}`}>
          <div className=' w-full bg-transparent h-full flex items-center flex-col justify-center'>
            <div className={`flex items-center gap-3 text-white mb-5 absolute top-[20px] bg-transparent justify-center ${changeMethod ? 'hidden':'left-[20px] '}`}>
              <img src={logo} alt="" className='w-11' />
              <h1 className='text-xl font-bold font-Gabarito'>Absensi Wikrama</h1>
            </div>
            <h1 className='text-[#FFDE59] text-5xl font-bold font-Gabarito text-center'>Welcome</h1>
            ...
            <div className='flex items-center mt-10 mb-7 gap-2 w-full' style={{ whiteSpace: 'nowrap' }}>
                <span className='w-full h-[1px] bg-[#FFDE59]'/>
                <h1 className='text-[#FFDE59] w-[316px] text-overflow-ellipsis'>Login With Your Email</h1>
                <span className='w-full h-[1px] bg-[#FFDE59]'/>
              </div>
            <div className='w-full flex flex-col gap-5'>
              ...
              <div className='flex text-white'>
                ...
                <input
                  ...
                  className={`placeholder:text-white w-full h-20 text-xl  bg-transparent outline-none p-2 border-b border-t ${password ? '':' border-r rounded-r-lg'}`}
                />
                {password && (
                  <button
                    type='button'
              ...
              <div className='flex items-center mb-5 mt-5 gap-2 w-full' style={{ whiteSpace: 'nowrap' }}>
                <span className='w-full h-[1px] bg-[#FFDE59]'/>
                <h1 className='text-[#FFDE59] w-[316px] text-overflow-ellipsis'>Another Option</h1>
                <span className='w-full h-[1px] bg-[#FFDE59]'/>
              </div>
              <div className='w-full text-white flex justify-between'>
                <button className='capitalize' onClick={toggleChangeMethod} >Not Have Account</button>
                <button className='capitalize'>forgot password?</button>
              </div>
              <button className='w-full h-20 bg-[#FFDE59] text-white font-bold rounded-md'>Submit</button>
            </div>
          </div>
        </div>
      </div>
        <div
        className={`bg-transparent h-full text-white duration-[380ms] ${changeMethod ? 'w-[800px] ' : 'w-[100%]  custom-background '}`}
      >
        <div className={` w-full h-full px-5 ${changeMethod ? 'opacity-100 ' : 'opacity-0 hidden'}`}>
          <div className=' w-full bg-transparent h-full flex items-center flex-col justify-center'>
            <div className={`flex items-center gap-3 text-white mb-5 absolute top-[20px] bg-transparent justify-center ${changeMethod ? 'hidden':'left-[20px] '}`}>
              <img src={logo} alt="" className='w-11' />
              <h1 className='text-xl font-bold font-Gabarito'>Absensi Wikrama</h1>
