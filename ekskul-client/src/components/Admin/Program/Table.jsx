...
const TableEskul = ({ setFormOld, setOpen }) => {
  ...
  const handleGetRequest = async () => {
    try {
      const response = await getAllActivityProgram({ filter, sort, size, number});

      if (response && response.data) {
        console.log('API Response:', response.data);
        console.log(response);

        if (Array.isArray(response.data.activityProgramFilter)) {
          const responseData = response.data.activityProgramFilter;
