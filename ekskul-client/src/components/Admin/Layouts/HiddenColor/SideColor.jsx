...
const SideColor = ({ toggleOpenChangeBg }) => {
  ...
  useEffect(() => {
    const savedColor = localStorage.getItem('sideColors');
    if (savedColor) {
      setBackgroundColor(savedColor);
      setSavedBackgroundColor(savedColor);
    }
  
    const savedCustomColors = JSON.parse(localStorage.getItem('sideCustomColors')) || [];
  ...
  const handleSave = () => {
    localStorage.setItem('sideBackgroundColor', backgroundColor);
  ...
  const handleReset = () => {
    ...
    localStorage.setItem('sideBackgroundColor', defaultColor);
    window.location.reload(); 
  };
  ...
  const handleAddCustomColor = () => {
    ...
    if (isValidFormat) {
      ...
      localStorage.setItem('sideCustomColors', JSON.stringify(updatedColorList));
    ...
    <div className='flex'>
        <div className='w-[100%] p-3'>
          <div className='flex flex-col w-full'>
            <button onClick={toggleOpenChangeBg}>Side Background</button>
