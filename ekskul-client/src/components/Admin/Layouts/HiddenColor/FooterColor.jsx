...
const FooterColor = ({ toggleOpenChangeBg }) => {
  ...
  useEffect(() => {
    const savedColor = localStorage.getItem('footerColors');
    if (savedColor) {
      setBackgroundColor(savedColor);
      setSavedBackgroundColor(savedColor);
    }
  
    const savedCustomColors = JSON.parse(localStorage.getItem('footerCustomColors')) || [];
  ...
  const handleSave = () => {
    localStorage.setItem('footerBackgroundColor', backgroundColor);
  ...
  const handleAddCustomColor = () => {
    ...
    if (isValidFormat) {
      ...
      localStorage.setItem('footerCustomColors', JSON.stringify(updatedColorList));
    ...
    <div className='flex'>
        <div className='w-[100%] p-3'>
          <div className='flex flex-col w-full'>
            <button onClick={toggleOpenChangeBg}>Footer Background</button>
