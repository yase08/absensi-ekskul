...
const TableEskul = () => {
  ...
  const [rombelOptions, setRayonOptions] = useState([]);
  ...
  const handleGetRequest = async () => {
    try {
      ...
      if (response && response.data) {
        ...
        if (Array.isArray(response.data.rombelFilter)) {
          const rombelData = response.data.rombelFilter;
          const rombels = response.data.rombels;
          setData(rombelData);
  
          // Filter the rombel options based on your criteria
          const uniqueOptions = {};
          ...
          setGetAllData(response.data.rombels.length);
        } else {
      ...
      <div className="overflow-x-auto hidden-scroll w-full">
        <table className="min-w-full border-collapse w-full bg-transparent">
          <thead>
            ...
            >
              {rombelOptions.map((option, index) => (
